...
class SparseMultiHeadAttention(nn.Module):
    ...
    def forward(self, x: Union[SparseTensor, torch.Tensor], context: Optional[Union[SparseTensor, torch.Tensor]] = None,
        ...
        if self._type == "self":
            ...
            # KV融合逻辑：如果提供了precomputed_k/v，按需处理
            if (precomputed_k is not None) and (precomputed_v is not None):
                # 直接不融合：忽略precomputed_k/v，使用auto的K/V
                # 以下为原融合实现，保留为注释，便于回滚/对照
                # --- 原融合实现开始 ---
                # q, k_auto, v_auto = qkv.unbind(dim=1)
                # ...（构造k_ref/v_ref并进行加权或concat融合）...
                # --- 原融合实现结束 ---

                # 改为：沿用auto K/V路径，与未提供precomputed时一致
                q, k_auto, v_auto = qkv.unbind(dim=1)
                self.last_k = k_auto
                self.last_v = v_auto
                
                if self.attn_mode == "full":
                    h = sparse_scaled_dot_product_attention(q=q, k=k_auto, v=v_auto)
