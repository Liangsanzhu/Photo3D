...
def intrinsics_to_projection(
    ...
    """
    # Ensure intrinsics is a 2D tensor with shape [3, 3]
    if intrinsics.dim() == 1:
...
def render(viewpoint_camera, pc : Gaussian, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    # subpixel_offset 也要在同一 device
    subpixel_offset = torch.zeros((int(viewpoint_camera.image_height), int(viewpoint_camera.image_width), 2), dtype=torch.float32, device=device)

    # 保证 bg_color 也在同一 device
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        viewmatrix=viewpoint_camera.world_view_transform.float().to(device),
        projmatrix=viewpoint_camera.full_proj_transform.float().to(device),
        sh_degree=pc.active_sh_degree,
        campos=viewpoint_camera.camera_center.float().to(device),
        prefiltered=False,
        debug=pipe.debug
    )
    """
    ...
    # Ensure critical tensors in raster_settings are float32
    """
    if hasattr(raster_settings, 'viewmatrix') and raster_settings.viewmatrix is not None:
        raster_settings.viewmatrix = raster_settings.viewmatrix.float()
    if hasattr(raster_settings, 'projmatrix') and raster_settings.projmatrix is not None:
        raster_settings.projmatrix = raster_settings.projmatrix.float()
    if hasattr(raster_settings, 'campos') and raster_settings.campos is not None:
        raster_settings.campos = raster_settings.campos.float()
    """
    # bg_color is assumed to be float32 from its creation due to earlier initialization logic.
    ...
    #print(means3D.dtype,means2D.dtype,shs.dtype)
    #print(opacity.dtype,scales.dtype,rotations.dtype)

    # Rasterize visible Gaussians to image, obtain their radii (on screen). 
    #rendered_image, radii = rasterizer(
    rendered_image, radii, rendered_depth, rendered_alpha= rasterizer(
        means3D = means3D.float() if means3D is not None else None,
        means2D = means2D.float() if means2D is not None else None,
        shs = shs.float() if shs is not None else None,
        colors_precomp = colors_precomp.float() if colors_precomp is not None else None,
        opacities = opacity.float() if opacity is not None else None,
