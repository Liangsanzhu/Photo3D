...
class Gaussian:
    ...
    def load_ply(self, path, transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]]):
        ...
        if transform is not None:
            # Apply the inverse of the transform that is used in save_ply so
            # that loading a PLY recovers the original Gaussian orientation.
            # In save_ply we store:
            #   xyz' = xyz @ transform.T
            #   R'   = transform @ R
            # Here we invert this:
            #   xyz  = xyz' @ transform
            ...
            # rots: (N,4) quaternions -> (N,3,3) rotation matrices
            rot_mats = utils3d.numpy.quaternion_to_matrix(rots)
            # apply inverse transform (transpose) on the left
            rot_mats = np.matmul(transform.T, rot_mats)
            rots = utils3d.numpy.matrix_to_quaternion(rot_mats)
