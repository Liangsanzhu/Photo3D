...
def cycle(data_loader: DataLoader) -> Iterator:
    """Infinite dataloader that reshuffles EVERY STEP.
    为了避免总是同一条样本，每次仅取一个batch就结束迭代并提升epoch。"""
    print("Entering cycle function...")
    iteration_count = 0
    while True:
        iteration_count += 1
        # 取一个batch
        ...
        if iteration_count <= 5:
            try:
                ...
                if isinstance(iid, list) and len(iid) > 0:
                    iid = iid[0]
                print(f"Cycle iteration {iteration_count}: Yielding 1 batch, instance_id={iid}")
            except Exception:
                print(f"Cycle iteration {iteration_count}: Yielding 1 batch")
        yield data
        # 每步结束 bump 一次 epoch，使下次取batch前重排
...
class BalancedResumableSampler(ResumableSampler):
    ...
    def __iter__(self) -> Iterator:
        #print(f"BalancedResumableSampler.__iter__ called, dataset length: {len(self.dataset)}")
        if self.shuffle:
            # deterministically shuffle based on epoch and seed
            g = torch.Generator()
            g.manual_seed(self.seed + self.epoch)
            #print("before torch.randperm")
           # print(f"len(self.dataset): {len(self.dataset)}")
            #print(f"type of len(self.dataset): {type(len(self.dataset))}")
            indices = torch.randperm(len(self.dataset), generator=g).tolist()  # type: ignore[arg-type]
            #print("after torch.randperm")
        else:
            indices = list(range(len(self.dataset)))  # type: ignore[arg-type]
        ...
        #print(f"num_batches: {num_batches}, batch_size: {self.batch_size}, world_size: {self.world_size}")
        ...
        if num_batches > 0:
            ...
            # 如果数据集太小，直接返回所有indices
           # print("Dataset too small for batch balancing, returning all indices")
            balanced_indices = indices
        
        # resume from previous state
        indices = balanced_indices[self.idx:]

        #print(f"Final balanced_indices length: {len(indices)}")
        #print(f"Final indices length: {indices}")
        return iter(indices)
