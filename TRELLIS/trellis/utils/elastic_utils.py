...
class LinearMemoryController(MemoryController):
    ...
    def record(self):
        ...
        self._last_mem_ratio = sum(self._last_mem_ratio) / len(self._last_mem_ratio) if len(self._last_mem_ratio) > 0 else 0.0
    ...
    def _fit_params(self):
        ...
        try:
            ...
            # 如果polyfit失败，使用简单的线性近似
            if len(x) > 1 and np.std(x) > 1e-10:
                k = np.mean(y) / np.mean(x) if np.mean(x) > 1e-10 else 0.0
                b = np.mean(y) - k * np.mean(x)
            else:
                k, b = 0.0, np.mean(y) if len(y) > 0 else 0.0
            self._params = (k, b)
            # 可选：输出警告
            # print(f"警告: elastic memory polyfit失败 ({e})，使用简单线性近似")
        # self._visualize()
    ...
    def get_mem_ratio(self, input_size):
        k, b = self._params
        # 保护：避免除0或非有限值
        if not np.isfinite(k) or k == 0 or input_size in [0, None]:
            return np.random.rand() * self._max_mem_ratio
        denom = k * input_size
        if denom == 0 or not np.isfinite(denom):
            return np.random.rand() * self._max_mem_ratio
        pred = (self.available_memory * self.target_ratio - b) / denom
        if not np.isfinite(pred):
            pred = self._max_mem_ratio
        return float(min(self._max_mem_ratio, max(0.0, pred)))
