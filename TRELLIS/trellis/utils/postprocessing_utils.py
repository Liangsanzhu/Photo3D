...
def to_glb(
    ...
    # Convert observations (which may be torch tensors in CHW format) to NumPy HWC uint8
    obs_np = []
    masks = []
    for obs in observations:
        if isinstance(obs, torch.Tensor):
            obs = obs.detach().cpu().numpy()
        # If CHW, move channels to last dimension
        if obs.ndim == 3 and obs.shape[0] in (1, 3, 4):
            obs = np.transpose(obs, (1, 2, 0))
        # Ensure uint8 in [0, 255]
        if np.issubdtype(obs.dtype, np.floating):
            obs = np.clip(obs * 255.0, 0, 255).astype(np.uint8)
        elif obs.dtype != np.uint8:
            obs = obs.astype(np.uint8)
        obs_np.append(obs)
        masks.append(np.any(obs > 0, axis=-1))
