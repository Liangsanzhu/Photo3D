...
class Converter(nn.Module):
        ...
        for (ver, hor) in zip(vers, hors):
            ...
            for i in pbar:
                ...
                if opt.save_image:
                    ...
                    kiui.write_image(f'{self.opt.output_dir}/{opt.texture_name}/albedo_gt_images/{i}.jpg', image_gt_save)

                    image_pred_save = image_pred.detach().cpu().numpy()
                    image_pred_save = image_pred_save.transpose(1, 2, 0)
                    kiui.write_image(f'{self.opt.output_dir}/{opt.texture_name}/mesh_albedo_images/{i}.jpg', image_pred_save)
            ...
            for (ver, hor) in zip(vers, hors):
                ...
                for i in pbar:
                    ...
                    if opt.save_image:
                        image_gt_save = image_gt.detach().cpu().numpy()
                        image_gt_save = image_gt_save.transpose(1, 2, 0)
                        kiui.write_image(f'{self.opt.output_dir}/{opt.texture_name}/material_gt_images/{i}.jpg', image_gt_save)

                        image_pred_save = image_pred.detach().cpu().numpy()
                        image_pred_save = image_pred_save.transpose(1, 2, 0)
                        kiui.write_image(f'{self.opt.output_dir}/{opt.texture_name}/mesh_material_images/{i}.jpg', image_pred_save)
    ...
    def update_text_embedding(self, text):
        token = tokenize(text)
        token = token.to(self.device)
        self.text_embedding = self.model.text_encoder.encode(token).float()

    @torch.no_grad()
    def save_gaussian_prid_grid(self):
        # Ensure gaussians are prepared
        cond = self.text_embedding if hasattr(self, 'text_embedding') else None
        if self.opt.use_material:
            _, self.gaussians, self.mr_gaussians = self.model.forward_gaussians(
                self.input_data, self.octree_in, condition=cond, data=None, ema=True
            )
        else:
        ...
        # Define 4 canonical views: left, front, right, back
        vers = [0, 0, 0, 0]
        hors = [-90, 0, 90, 180]
        rad = self.opt.texture_cam_radius

        images = []
        for ver, hor in zip(vers, hors):
            pose = orbit_camera(ver, hor, rad)
            image, _ = self.render_gs(pose, use_material=False)
            img = image.detach().cpu().numpy().transpose(1, 2, 0)  # H, W, C
        ...
        # Extract id from mesh_path: {id}_final.glb
        base = os.path.basename(self.opt.mesh_path)
        m = re.match(r'(.+?)_final\\.glb$', base)
        if m:
            id_str = m.group(1)
        else:
        ...
        out_path = os.path.join(self.opt.output_dir, f'{id_str}_prid.png')
        kiui.write_image(out_path, grid)
        print(f'[INFO] Saved Gaussian 4-view grid to: {out_path}')
...
if __name__ == "__main__":
    ...
    # Batch mode: if mesh_path is a directory, iterate all *_final.glb and fetch text from fixed captions json.
    if os.path.isdir(opt.mesh_path):
        captions_json = '/home/xinyue_liang/lxy/eval_data/imagenet-1k/captions_blip2.json'
        with open(captions_json, 'r') as f:
            captions = json.load(f)
        glb_paths = sorted(glob.glob(os.path.join(opt.mesh_path, '*_final.glb')))
        print(f'[INFO] Found {len(glb_paths)} glb files under {opt.mesh_path}')
        for glb_path in glb_paths:
            base = os.path.basename(glb_path)
            m = re.match(r'(.+?)_final\\.glb$', base)
            id_str = m.group(1) if m else os.path.splitext(base)[0]
            text = captions.get(id_str, None)
            if text is not None:
                converter.update_text_embedding(text)
            else:
                print(f'[WARN] No caption found for id {id_str}, proceeding without text condition.')
                if hasattr(converter, 'text_embedding'):
                    delattr(converter, 'text_embedding')
            converter.load_mesh(glb_path)
            # Override mesh_path for correct output naming
            converter.opt.mesh_path = glb_path
            converter.save_gaussian_prid_grid()
    else:
        converter.load_mesh(opt.mesh_path)
        if opt.save_image:
            # Only save Gaussian four-view grid image and skip mesh texturing
            converter.save_gaussian_prid_grid()
        else:
